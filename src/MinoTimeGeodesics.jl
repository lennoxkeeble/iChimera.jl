...
function compute_kerr_geodesic_manual(a::Float64, p::Float64, e::Float64, θmin::Float64, sign_Lz::Int64, specify_params::Bool, saveat_λ::AbstractVector{Float64}, Δλi::Float64=1.0, reltol::Float64=1e-10, abstol::Float64=1e-10;
    ics::SVector{4, Float64}=SA[0.0, 0.0, 0.0, 0.0], E::Float64=0.0, L::Float64=0.0, Q::Float64=0.0, C::Float64=0.0, ra::Float64=0.0, p3::Float64=0.0, p4::Float64=0.0, zp::Float64=0.0, zm::Float64=0.0)
    ...
    # initial conditions for Kerr geodesic trajectory
    λmax = maximum(saveat_λ);
    λspan = (0.0, λmax);
    nPoints = length(saveat_λ)
    ...
    # check initial conditions satisfied
    if !isapprox(ics, sol(0.0), atol=1e-10)
        throw(ArgumentError("Initial conditions not satisfied. ics = $(ics), sol(0.0) = $(sol(0.0))"))
    end
