...
function compute_coefficient_matrix_with_derivs(nFreqs::Int, nParams::Int, nPoints::Int, xdata::Vector{<:Number}, fit_freqs::Vector{<:Number})::AbstractArray
    A = ones(2 * nPoints, nParams)
    @inbounds for i in 1:nPoints
        @inbounds for j in 1:nFreqs
            A[i, 2*j] = cos(fit_freqs[j] * xdata[i])
            A[i, 2*j + 1] = sin(fit_freqs[j] * xdata[i])

            # first time derivative of the fourier series
            A[nPoints + i, 2*j] = -fit_freqs[j] * sin(fit_freqs[j] * xdata[i])
            A[nPoints + i, 2*j + 1] = fit_freqs[j] * cos(fit_freqs[j] * xdata[i])
        end
    end
    return A
end

# fit using derivatives as an additional constraint
# master functions for carrying out fit with one, two or three fundamental frequencies
function Fit_1_freqs_with_derivs!(xdata::Vector{<:Number}, ydata::Vector{<:Number}, nPoints::Int, nHarm::Int,  Ω1::Number, fit_params::Vector{<:Number})::Vector{<:Number}
    # compute fitting frequncies and their Float64
    Ω_fit = FourierFitJuliaBase.compute_fitting_frequencies_1(nHarm, Ω1)
    n_freqs = compute_num_fitting_freqs_1(nHarm)
    n_coeffs = 2 * n_freqs + 1    # +1 to allocate memory for the constant term, and factor of 2 since we have sin and cos for each frequency
    ...
    # allocate memory and fill GSL vectors and matrices
    coeff_matrix = compute_coefficient_matrix_with_derivs(n_freqs, n_coeffs, nPoints, xdata, Ω_fit)
    @views fit_params[:] = coeff_matrix \ ydata
    return Ω_fit
end

function Fit_2_freqs_with_derivs!(xdata::Vector{<:Number}, ydata::Vector{<:Number}, nPoints::Int, nHarm::Int, Ω1::Number, Ω2::Number, fit_params::Vector{<:Number})::Vector{<:Number}
    # compute fitting frequncies and their Float64
    Ω_fit = FourierFitJuliaBase.compute_fitting_frequencies_2(nHarm, Ω1, Ω2)
    n_freqs = compute_num_fitting_freqs_2(nHarm)
    n_coeffs = 2 * n_freqs + 1    # +1 to allocate memory for the constant term, and factor of 2 since we have sin and cos for each frequency
...
function Fit_3_freqs_with_derivs!(xdata::Vector{<:Number}, ydata::Vector{<:Number}, nPoints::Int, nHarm::Int, Ω1::Number, Ω2::Number, Ω3::Number, fit_params::Vector{<:Number})::Vector{<:Number}
    # compute fitting frequncies and their Float64
    Ω_fit = FourierFitJuliaBase.compute_fitting_frequencies_3(nHarm, Ω1, Ω2, Ω3)
    n_freqs = compute_num_fitting_freqs_3(nHarm)
    n_coeffs = 2 * n_freqs + 1    # +1 to allocate memory for the constant term, and factor of 2 since we have sin and cos for each frequency
...
function Fit_master_with_derivs!(xdata::Vector{<:Number}, ydata::Vector{<:Number}, nPoints::Int, nHarm::Int, Ω::Vector{<:Number}, fit_params::Vector{<:Number})::Vector{<:Number}
    freqs = Ω[Ω .< 1e9];
    num_freqs = length(freqs);
    if num_freqs==1
        return Fit_1_freqs_with_derivs!(xdata, ydata, nPoints, nHarm, freqs..., fit_params)
    elseif num_freqs==2
        return Fit_2_freqs_with_derivs!(xdata, ydata, nPoints, nHarm, freqs..., fit_params)
    elseif num_freqs==3
        return Fit_3_freqs_with_derivs!(xdata, ydata, nPoints, nHarm, freqs..., fit_params)
    end
end
